...
Copy this file to `explorer.py` and fill in every section marked `# TODO:`.

The code is structured in 5 sections that match the Step_by_Step guide exactly.
Each section has working code around the TODOs so you can run and test incrementally.
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # TODO: Use df.dropna() to drop rows where 'Age' is missing.
    # Hint: df.dropna(subset=['Age']) keeps only rows with a valid Age.
    # Store the result in a new variable called df_clean and return it.
...
def compute_statistics(df: pd.DataFrame, column: str) -> dict:
    ...
    """
    values = df[column].values  # Convert to numpy array
...
def print_marginal_probabilities(df: pd.DataFrame) -> None:
    ...
    # TODO: Call marginal_probability() for each of these and print the result:
    # P(Survived=1), P(Sex='female'), P(Pclass=1)
    # Format each line like: f"P(Survived)     = {p:.4f}"

    # TODO: Fill in these three print statements
    # P(Survived)
    p_survived = marginal_probability(df, "Survived", 1)
    print(f"P(Survived)            = {p_survived:.4f}")

    # P(Female)
    p_female = None  # TODO: call marginal_probability for Sex='female'
    print(f"P(Female)              = {p_female:.4f}")

    # P(Pclass=1)
    p_first = None  # TODO: call marginal_probability for Pclass=1
...
def conditional_probability(df: pd.DataFrame,
    ...
    This is: "given that condition_col equals condition_val,
              what fraction of those rows have target_col == target_val?"

    Args:
        df: the full dataset
        target_col: the column we want the probability of
        target_val: the value we're checking in target_col
        condition_col: the column we're conditioning on
        condition_val: the value to filter condition_col to
    ...
    # Hint: use (subset[target_col] == target_val).sum() / len(subset)
    probability = None  # TODO: replace this line
...
def print_conditional_probabilities(df: pd.DataFrame) -> None:
    """Print conditional survival probabilities."""
...
def plot_histogram(df: pd.DataFrame, column: str, output_dir: str) -> None:
    """
    Plot a histogram of the given column and save it to output_dir.
...
def plot_survival_by_class(df: pd.DataFrame, output_dir: str) -> None:
    ...
    # TODO: Use df.groupby('Pclass')['Survived'].mean() to get survival rates per class
    ...
    # TODO: Set plt.xticks(rotation=0) so labels are horizontal
...
|---|---|
| `clean_data()` | Use `dropna(subset=['Age'])` to remove rows with missing Age |
| `compute_statistics()` | Use `np.mean()`, `np.var()`, `np.std()` |
| `marginal_probability()` | Count matching rows, divide by total |
| `conditional_probability()` | Filter df, then calculate proportion |
| `plot_histogram()` | Call `plt.hist()` with title/labels |
| `plot_survival_by_class()` | Use `groupby().mean()` then `.plot(kind='bar')` |
...
If your numbers match, your conditional probability implementation is correct.
