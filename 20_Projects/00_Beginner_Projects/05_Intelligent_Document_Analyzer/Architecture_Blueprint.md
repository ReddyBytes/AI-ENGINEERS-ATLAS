# Project 5 — Architecture Blueprint

## System Overview

This project is a **prompt chain pipeline** — multiple LLM API calls, each with a specific prompt and output format, wired together into a single application. The document acts as shared context across all 4 analysis tasks.
...
Note: These strategies reduce hallucination but cannot eliminate it. LLMs can still generate plausible-sounding but incorrect answers even with strong grounding instructions. For high-stakes applications, answers should always be verified against source material.
...
```
05_Intelligent_Document_Analyzer/
├── analyzer.py               ← Your main Python script
├── test_document.txt         ← A text file you want to analyze
├── Project_Guide.md
├── Step_by_Step.md
├── Starter_Code.md
...
```mermaid
flowchart TD
    T26[Topic 26 — Prompt Engineering] --> C1[4 task-specific prompts\nwith output constraints]
    T27[Topic 27 — LLM APIs] --> C2[call_api function\nmessages format + max_tokens]
    T28[Topic 28 — Structured Outputs] --> C3[JSON schema in prompt\npydantic validation]
    T25[Topic 25 — Hallucination] --> C4[Grounding instructions\nin every prompt]
    T29[Topic 29 — Embeddings] --> C5[Context: why chunking needed\nfor very long documents]
