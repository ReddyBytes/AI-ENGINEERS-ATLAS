...
Copy this file to `compare_models.py` and fill in every section marked `# TODO:`.
...
def load_and_split(test_size: float = 0.2, random_state: int = 42):
    ...
    # IMPORTANT: also set stratify=y to keep class balance in both sets
...
def get_models() -> dict:
    ...
    All models are initialized but NOT yet trained.
    """
    # TODO: Fill in the four models below.
    # Use these classes: LogisticRegression, DecisionTreeClassifier,
    # RandomForestClassifier, GaussianNB
    # Set random_state=42 on models that accept it.
    # For RandomForestClassifier, set n_estimators=100.
    # For LogisticRegression, set max_iter=200.
...
def train_all_models(models: dict, X_train, y_train) -> None:
    ...
    for name, model in models.items():
        ...
        # TODO: print a message like f"Trained: {name}"
        pass  # TODO: replace
...
def evaluate_model(model, X_test, y_test) -> dict:
    ...
    # For precision, recall, and f1: use average='weighted'
    accuracy  = None  # TODO: accuracy_score(y_test, y_pred)
    precision = None  # TODO: precision_score(...)
...
def build_comparison_table(models: dict, X_test, y_test) -> pd.DataFrame:
    """
    Evaluate all models and return a pandas DataFrame with results.
    One row per model, columns: Model, Accuracy, Precision, Recall, F1
    """
    rows = []
    for name, model in models.items():
        # TODO: Call evaluate_model(model, X_test, y_test)
        # Add the result dict to rows, plus a 'Model' key with the model name
...
def check_overfitting(models: dict, X_train, y_train, X_test, y_test) -> None:
    """
    Print the train accuracy, test accuracy, and gap for each model.
    Flag any model with a gap > 0.05 as a potential overfit.
    """
    print("--- Train vs Test Accuracy (Overfitting Check) ---")
    for name, model in models.items():
        # TODO: Use model.score(X_train, y_train) to get train accuracy
        # TODO: Use model.score(X_test, y_test) to get test accuracy
...
|---|---|
| `load_and_split()` | Call `train_test_split()` with stratify=y |
| `get_models()` | Instantiate all 4 sklearn model objects |
| `train_all_models()` | Call `model.fit()` for each model |
| `evaluate_model()` | Call `model.predict()`, then all 4 metric functions |
| `check_overfitting()` | Call `model.score()` on both train and test |
| `plot_confusion_matrix()` | Call `model.predict()` and `confusion_matrix()` |
| `find_best_model()` | Use `df.loc[df['F1'].idxmax(), 'Model']` |
...
Expected F1 scores (approximate, may vary slightly by sklearn version):
- Logistic Regression: ~0.97
- Decision Tree: ~0.93
- Random Forest: ~0.97
- Naive Bayes: ~0.97

The Decision Tree should show the largest train-test gap (~0.06 or more), indicating overfitting.
