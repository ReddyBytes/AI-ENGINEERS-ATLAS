# Project 2 — Personal Knowledge Base RAG: Starter Code

## How to Use This File

Copy the code below into `rag_pipeline.py`. Every `# TODO:` marks something you must implement. The scaffolding is complete — function signatures, docstrings, and flow control are provided. You fill in the bodies.
...
Usage:
    python rag_pipeline.py --ingest    # index documents in ./documents/
    python rag_pipeline.py             # run Q&A interface
...
def chunk_text(
    ...
    #           append {"text": chunk_text, "char_start": start, "char_end": end}
...
def index_documents(documents_dir: Path = DOCUMENTS_DIR) -> None:
    ...
    for source_file, file_pages in files.items():
        ...
        # Collect all chunks for this file across all pages
        all_chunks = []
        ...
        embeddings = []  # replace with your loop above
        ...
        #   metadatas:  list of metadata dicts (everything except "text")
        #               build each as: {k: v for k, v in c.items() if k != "text"}
...
def retrieve(
    ...
    #      )
    #   3. results["documents"][0] is the list of chunk texts
    #      results["metadatas"][0] is the list of metadata dicts
    #      results["distances"][0] is the list of cosine distances [0, 2]
    #   4. Convert distance to similarity: score = 1 - (distance / 2)
    #   5. Build and return a list of result dicts merging text, score, and metadata.
    pass
...
After implementing all TODOs, run these tests to validate each stage:
...
assert all(c["char_end"] - c["char_start"] <= 200 for c in chunks), "Chunk too large"
# Verify overlap: end of chunk N should overlap with start of chunk N+1
for i in range(len(chunks) - 1):
    assert chunks[i]["char_end"] > chunks[i+1]["char_start"], "No overlap between chunks"
print("Chunking test passed.")

# Test embedding
vecs = embed_texts(["hello world", "foo bar"])
assert len(vecs) == 2, "Wrong number of embeddings"
assert len(vecs[0]) == 1536, f"Wrong embedding dimension: {len(vecs[0])}"
print("Embedding test passed.")

# Test cosine distance → similarity conversion
# ChromaDB cosine distance 0.0 should become score 1.0
# ChromaDB cosine distance 2.0 should become score 0.0
score_0 = 1 - (0.0 / 2)   # 1.0
score_2 = 1 - (2.0 / 2)   # 0.0
assert score_0 == 1.0 and score_2 == 0.0
...
**PDF extraction returns empty strings**: The PDF is likely image-based (scanned). `pdfplumber` cannot read scanned PDFs without OCR. Skip those files or use `pytesseract`.

**ChromaDB duplicate IDs**: If you run `--ingest` twice on the same files without the skip check, you get an error. The `is_file_indexed()` guard prevents this.

**Mixing embedding models**: The query must be embedded with the same model as the documents. A mismatch produces garbage retrieval with no error message.

**Claude hallucinating despite context**: Check your system prompt. Adding "Answer ONLY using the provided sources" is necessary but not sufficient — also verify the relevant chunks were actually retrieved.
