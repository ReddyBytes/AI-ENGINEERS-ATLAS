# Project 1 — Semantic Search Engine: Architecture Blueprint

## System Overview

The semantic search engine is a two-phase system: an offline **indexing phase** that embeds documents once, and an online **query phase** that embeds the user's query and finds nearest neighbors.
...
| 10 docs × 300 words each | ~$0.0001 total | One-time at index time |
...
The indexing phase is done once (or when documents change). All search latency after that is sub-millisecond numpy operations plus one API call for the query.
...
This architecture works for up to a few thousand documents on a laptop. To scale further:
