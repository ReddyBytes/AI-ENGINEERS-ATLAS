# Project 4 — Custom LoRA Fine-Tuning: Architecture Blueprint
...
LoRA does not modify W. It adds a parallel path through two small matrices A and B. During inference, the LoRA contribution `(α/r) × B·A·x` is added to the frozen model's output. After training, you can merge: `W_merged = W + (α/r) × B·A`, eliminating the extra computation.
...
```mermaid
pie title Trainable Parameters: Full Fine-tuning vs LoRA (TinyLlama 1.1B)
    "LoRA adapter (trainable): 5M" : 0.46
    "Frozen base model: 1.1B" : 99.54
```
...
| Tokenizer | `AutoTokenizer` | Text → token IDs | pad_token = eos_token |
...
Users who want to use your model download these two files and call `PeftModel.from_pretrained(base_model, "your-username/tinyllama-ml-tutor-lora")`. They do not need to re-download the 1.1B base model if they already have it cached.
