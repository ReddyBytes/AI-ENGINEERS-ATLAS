...
|---|---|---|---|---|
| Supervisor (Plan) | Decompose question → sub-tasks | Claude claude-sonnet-4-6 | `research_question` | `sub_tasks` (list of task dicts) |
...
| Research Question | Web Search Task | Wikipedia Task |
|---|---|---|
| "AI impact on manufacturing employment" | "AI manufacturing job displacement 2024" | "Industry 4.0" |
| "Climate change effects on agriculture" | "climate change crop yield 2023 study" | "Climate change and agriculture" |
| "CRISPR gene editing applications" | "CRISPR medical treatment 2024 trials" | "CRISPR" |
...
Key insight: each `Send` gives the worker a **copy** of the full state plus the specific task. Workers return **partial state updates** (only the fields they modify). LangGraph merges these partial updates using the field-level merge strategies (operator.add for lists).
...
├── research_system.py     # Main system (from Starter_Code.md)
├── research_output/       # Generated reports (auto-created)
│   └── report_20260314_143022.md
└── requirements.txt
```
