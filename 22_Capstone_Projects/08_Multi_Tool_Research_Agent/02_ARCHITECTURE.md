# Project 08 — Multi-Tool Research Agent: Architecture
...
sequenceDiagram
    ...
    App->>Claude: messages=[{role:"user", content:"Population of Tokyo vs Paris..."}]
    Claude-->>App: stop_reason="tool_use", content=[ToolUseBlock(id="tu_01", name="wikipedia_summary", input={"topic":"Tokyo"})]
    ...
    Claude-->>App: stop_reason="tool_use", content=[ToolUseBlock(id="tu_02", name="wikipedia_summary", input={"topic":"Paris"})]

    App->>App: execute_tool("wikipedia_summary", {"topic":"Paris"})
    App->>App: (append to history)

    App->>Claude: messages=[...full history...]
    Claude-->>App: stop_reason="tool_use", content=[ToolUseBlock(id="tu_03", name="calculator", input={"expression":"37.4/2.1"})]

    App->>App: execute_tool("calculator", {"expression":"37.4/2.1"})
    App->>App: (append to history)

    App->>Claude: messages=[...full history...]
    Claude-->>App: stop_reason="end_turn", content=[TextBlock("Tokyo (37.4M) is 17.8x larger than Paris (2.1M)...")]
...
| Tool: wikipedia_summary | `wikipedia_summary()` | Wikipedia article summaries | Requires close article title match |
...
| Layer | Tool | Why |
|---|---|---|
| LLM | Anthropic Claude (`claude-opus-4-6`) | Tool use support, strong reasoning |
| Web search | `duckduckgo-search` | Free, no API key required |
| Wikipedia | `wikipedia-api` | Clean structured summaries |
| Safe math | Python `ast` module | Evaluates math without `eval()` |
...
The quality of tool descriptions directly affects whether Claude picks the right tool.
