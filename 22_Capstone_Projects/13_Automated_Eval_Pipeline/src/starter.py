...
Usage:
    ...
    python src/starter.py --no-ragas --system-name "My RAG System"
"""
...
# Golden Dataset
# ─────────────────────────────────────────────
# These 20 examples are the ground truth your system is measured against.
# Four categories: factual, reasoning, safety, edge_case
# ─────────────────────────────────────────────
...
def query_system(question: str) -> tuple[str, list[str]]:
    ...
    For RAG systems: return the retrieved chunks as contexts.
    For chat-only systems: return empty list for contexts.
    """
    ...
    return response.content[0].text, []   # ← replace [] with real contexts for RAG
    ...
    def score(self, item: dict, answer: str) -> JudgeScore:
        ...
        Steps:
          1. Build the user message:
               ...
               AI Answer: {answer}
          2. Call Claude with JUDGE_SYSTEM_PROMPT as system, max_tokens=150
...
def run_ragas(golden_data: list[dict], system_outputs: list[tuple]) -> Optional[RagasScores]:
    ...
    Import inside function (ragas may not be installed):
    ...
    def compare(self, current_scores: dict, baseline: dict) -> list[RegressionResult]:
        ...
        Steps:
          1. For each metric key in current_scores:
               - Look up same key in baseline (skip if not present or key == "run_date")
          ...
          2. Print a formatted table:
               ...
               yellow (✓) = stable
               red    (⚠) = regression
          3. Return list of RegressionResult objects
        """
        results = []
...
class RedTeamEvaluator:
    ...
    def evaluate_attack(self, attack_id: str, category: str, attack_text: str) -> RedTeamResult:
        ...
        The judge must be an independent call — not the same SUT instance.
        """
        ...
        verdict = "BLOCKED"   # ← placeholder; implement the judge above
...
def generate_html_report(report: EvalReport) -> str:
    ...
    TODO: The template is already defined above as HTML_TEMPLATE.
    Steps:
      1. Import jinja2.Environment and BaseLoader
      2. Use env.from_string(HTML_TEMPLATE) to load the template
      3. Render with report object
