...
This is a **stateful CLI application** built around the Anthropic API. The key insight is that the model itself is stateless — it has no memory between API calls. All memory management happens in your code. You assemble the history, trim it, and send the full list on every request.
...
## Messages Array State Over Time
...
| Layer | Tool | Why |
|---|---|---|
| Language | Python 3.9+ | Standard library handles most needs |
| API client | `anthropic` SDK | Manages auth, HTTP, streaming context manager |
| Environment | `os.environ` | Reads API key without hardcoding |
| I/O | Python `input()` / `print()` | No UI framework needed for CLI |
...
```mermaid
flowchart TD
    T21[How LLMs Generate Text] --> C1[chat_streaming\nTokens printed one by one]
    T24[Context Windows] --> C2[trim_history\nKeep last N messages only]
    T26[Prompt Engineering] --> C3[SYSTEM_PROMPT constant\nShapes Claude behavior]
    T27[Using LLM APIs] --> C4[anthropic.Anthropic client\nmessages format user/assistant]
    T30[Streaming] --> C5[client.messages.stream\nstream.text_stream iterator]
