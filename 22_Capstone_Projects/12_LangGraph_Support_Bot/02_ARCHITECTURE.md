# Project 12 — LangGraph Support Bot: Architecture
...
| `intent` | `str or None` | One of the four intent classes | `classify` |
| `confidence` | `float` | 0.0–1.0, classifier confidence | `classify` |
| `retry_count` | `int` | Number of clarification retries | `clarify` |
| `order_id` | `str or None` | Extracted order ID if present | `classify`, `order_lookup` |
| `draft_response` | `str or None` | Pending escalation message for review | `escalation_draft` |
| `approved` | `bool` | Human approval for escalation | Set via `graph.update_state()` |
| `final_response` | `str or None` | The final bot message to the user | All specialist nodes |
...
|---|---|---|
| Graph framework | LangGraph (StateGraph) | State machine orchestration |
| State persistence | MemorySaver | Conversation checkpointing |
| LLM | Anthropic Claude (via langchain-anthropic) | Intent classification, response generation |
| Message types | langchain-core HumanMessage/AIMessage | Typed conversation history |
| Mock data | Python dict | Order database simulation |
